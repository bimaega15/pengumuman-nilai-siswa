...
<x-modal.modal-body>
    <div class="col-span-12 sm:col-span-12 mb-2">
        ...
        <input type="text" class="form-control" name="kode_datastatis" placeholder="Kode..."
            value="{{ isset($dataStatis) ? $dataStatis->kode_datastatis : '' }}">
    </div>
    <div class="col-span-12 sm:col-span-12 mb-2">
        <label for="" class="form-label">Nama Statis</label>
        <input type="text" class="form-control" name="nama_datastatis" placeholder="Nama Statis..."
            value="{{ isset($dataStatis) ? $dataStatis->nama_datastatis : '' }}">
    </div>
    <div class="col-span-12 sm:col-span-12 mb-2">
        ...
        <select name="jenisreferensi_datastatis" id="" class="form-select select2" style="width: 100%;">
            ...
            @foreach ($jenisReferensi as $value => $item)
                <option value="{{ $value }}"
                    {{ isset($dataStatis) ? ($dataStatis->jenisreferensi_datastatis == $value ? 'selected' : '') : '' }}>
                    {{ $item }}
                </option>
            @endforeach
        </select>
